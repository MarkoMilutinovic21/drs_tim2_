...
export const flightAPI = {
  create: (flightData) =>
    serverAPI.post('/api/flights', flightData),
  ...
  getAll: (filters = {}) => {
    ...
    return serverAPI.get(`/api/flights?${params.toString()}`);
  },
  
  getByTabs: () =>
    serverAPI.get('/api/flights/tabs'),
  
  getPending: () =>
    serverAPI.get('/api/flights/pending'),
  
  getById: (flightId) =>
    serverAPI.get(`/api/flights/${flightId}`),
  
  approve: (flightId) =>
    serverAPI.post(`/api/flights/${flightId}/approve`, { action: 'approve' }),
  
  reject: (flightId, reason) =>
    serverAPI.post(`/api/flights/${flightId}/approve`, {
      action: 'reject',
      rejection_reason: reason
    }),
  
  update: (flightId, flightData) =>
    serverAPI.put(`/api/flights/${flightId}`, flightData),
  
  cancel: (flightId) =>
    serverAPI.post(`/api/flights/${flightId}/cancel`),
  
  delete: (flightId) =>
    serverAPI.delete(`/api/flights/${flightId}`),

  generateReport: (reportType, adminId) =>
    serverAPI.post('/api/flights/report', {
...
export const bookingAPI = {
  create: (flightId, userId) =>
    serverAPI.post('/api/bookings', {
      flight_id: flightId,
      user_id: userId
    }),
  
  getById: (bookingId) =>
    serverAPI.get(`/api/bookings/${bookingId}`),
  
  getUserBookings: (userId) =>
    serverAPI.get(`/api/bookings/user/${userId}`),
  
  getFlightBookings: (flightId) =>
    serverAPI.get(`/api/bookings/flight/${flightId}`)
};

// ==================== RATING API ====================
export const ratingAPI = {
  create: (flightId, userId, rating, comment = null) =>
    serverAPI.post('/api/ratings', {
      flight_id: flightId,
      user_id: userId,
      rating,
      comment
  ...
  getById: (ratingId) =>
    serverAPI.get(`/api/ratings/${ratingId}`),
  
  getFlightRatings: (flightId) =>
    serverAPI.get(`/api/ratings/flight/${flightId}`),
  
  getAll: () =>
    serverAPI.get('/api/ratings')
};
